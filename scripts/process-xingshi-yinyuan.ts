/**
 * Processes Xingshi Yinyuan Zhuan (醒世姻緣傳) raw text into structured JSON.
 *
 * Source: 100 chapter files in data/raw/xingshi_yinyuan_zhuan/
 * Author: Xi Zhousheng (西周生, pseudonym, fl. 17th century)
 *
 * The raw file structure:
 * - One file per chapter: NNN_第N回[TITLE].txt (title embedded in filename)
 * - Each chapter contains: opening poem, prose narrative
 *
 * Each file becomes one chapter with paragraphs split on blank lines.
 *
 * Usage: pnpm tsx scripts/process-xingshi-yinyuan.ts
 */

import fs from "fs";
import path from "path";

const RAW_DIR = path.resolve("data/raw/xingshi_yinyuan_zhuan");
const OUTPUT_DIR = path.resolve("data/processed/xingshi-yinyuan-zhuan");
...
function extractChapterTitle(filename: string, chapterNum: number): string {
  // Filename format: 001_第一回晁大舍圍場射獵狐仙姑被箭傷生.txt
...
function main() {
  console.log("Processing Xingshi Yinyuan Zhuan (醒世姻緣傳)...\n");
